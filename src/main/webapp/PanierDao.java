...
public class PanierDao implements IDAO<Panier> {
	...
	public boolean create(Monture object) {
		...
		try {
			PreparedStatement req = connect.prepareStatement("INSERT INTO panier " + "(reference, quantite, prix, nom_produit, description, image)" + " VALUES (?,?,?,?,?,?) ");
			...
			req.setString(5, object.getDescription());
	...
	public List<Panier> read() {
		...
		try {
			...
			while(rs.next()) {
				...
				tempUser.setImage(rs.getString("image"));
